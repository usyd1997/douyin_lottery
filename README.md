...
[2026-1-30] 抖音福袋万花筒，抖音抢福袋工具，抖音无水印视频下载器，抖音直播间录制下载器。
...
v10.2.0
	...
	增加账号配置保存和读取功能。
	在多账号环境下，为每一个账号记录专属的软件设置，并在登陆时自动加载，省去每次配置的麻烦。
	该功能用户登录DY账号后，点击“保存设置”按钮，可以为该uid账号保存当前软件设置，当再次开启软件并点击“登录”该账号时，会自动加载该账号之前保存的设置。（软件运行5分钟后也会自动保存当前设置）
			
	增加只抢关注的直播间功能。
	该功能被勾选后，只会扫描关注列表中是否有主播正在发放福袋。
			
	优化红包功能，现在会自动屏蔽礼物红包，只抢钻石红包了。
	增加了自动化功能。
	该功能可以批量设置软件启动时间和关闭时间，自动设置账号，自动加载配置，自动开工，自动退出，适合工作室用户使用。（客户端已测试完成）
	该功能需要主控端配合使用。
	主控端将于一周后（2月8日左右）免费发布，请耐心等待。
